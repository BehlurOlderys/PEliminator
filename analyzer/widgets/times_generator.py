...
def get_data_from_correction_file(filepath):
    ...
    return old_times, old_data, old_model


def get_new_data_and_model(old_data, old_model, error_data):
...
def write_correction_data(times, new_data, new_model):
...
class TimesGenerator:
    ...
    def load(self):
        ...
        old_times, old_data, old_model = get_data_from_correction_file(self._old_path)
        new_data, new_model = get_new_data_and_model(old_data, old_model, self._data)
        write_correction_data(old_times, new_data, new_model)
