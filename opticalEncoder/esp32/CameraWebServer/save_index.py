...
index_html_name = "index_ov2640.html"
index_gzipped_name = "index_ov2640.html.gz"

with open(index_html_name, 'r') as index_f:
    html_str = index_f.read()

with gzip.open(index_gzipped_name, 'wb') as intermediate:
    with io.TextIOWrapper(intermediate, encoding='utf-8') as encode:
        encode.write(html_str)

with open(index_gzipped_name, 'rb') as secondary_f:
    new_hex_bytes = secondary_f.read()

# new_hex_ints = int.from_bytes(new_hex_bytes, 'little')
# hexes = list(map(hex, list(new_hex_bytes)))
hexes = list(map(lambda x: '0x{0:0{1}X}'.format(x,2), list(new_hex_bytes)))

print(hexes)

hex_length = len(hexes)

with open("camera_index.h", 'w') as output_f:
    output_f.write(f"#define index_ov2640_html_gz_len {hex_length}\n")
    output_f.write("const uint8_t index_ov2640_html_gz[] = {\n")

    i = 0
    while i < hex_length:
        max_hexes = min(16, hex_length-i)
        output_f.write(f"{', '.join(hexes[i:i+max_hexes])},\n")
        i += 16
